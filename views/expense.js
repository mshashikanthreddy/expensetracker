...
async function showExpenditure(event)
{
    ...
    try {
        ...
        const response = await axios.post('http://3.85.244.65:3000/expense/addExpense',expense,{headers : {'Authorization' : token}});
...
window.addEventListener('DOMContentLoaded' , async() => {
    ...
    console.log(document.getElementById('pagenumber').value );
        const response = await axios.get(`http://3.85.244.65:3000/expense/getExpense?page=1`,{headers : {'Authorization' : token,'ITEMS_PER_PAGE ': document.getElementById('pagenumber').value }})
...
async function fetchExpenses(page) {
    ...
    const response = await axios.get(`http://3.85.244.65:3000/expense/getExpense?page=${page}`,{headers : {'Authorization' : token,'ITEMS_PER_PAGE ': document.getElementById('pagenumber').value }});
    let expenses = response.data.expenses;
...
//   }
     ...
     async function deleteUser(id) {
        try {
            const token = localStorage.getItem('token');
          await axios.delete(`http://3.85.244.65:3000/expense/deleteExpense/${id}`,{headers : {'Authorization' : token}})
    ...
    async function showLeaderBoard() {
        ...
        inputElement.onclick = async() => {
            ...
            const token = localStorage.getItem('token');
            const leaderBoardList = await axios.get("http://3.85.244.65:3000/premium/leaderBoard", {headers : {"Authorization" : token}});
   ...
   async function download() {
            ...
            try {
            const token = localStorage.getItem('token');
         const response = await axios.get('http://3.85.244.65:3000/expense/download', { headers: { Authorization: token } })
...
document.getElementById('rzp-button1').onclick = async function (e) {
    ...
    const token = localStorage.getItem('token');
    const response = await axios.get("http://3.85.244.65:3000/purchase/premiumUser",{headers : {"Authorization" : token}});
    ...
    var options = 
    {
        ...
        "handler" : async function ( response) { // "handler" is a callback function which will handle the success payment
         ...
         const user =  await axios.post("http://3.85.244.65:3000/purchase/updateTransactionStatus",{
